...
本项目不应该自己承担云端存储账单。开源 MVP 让用户自带 GitHub repository（推荐 private；公开或私有由用户在 GitHub 选择），并通过 Skill Sync 的 GitHub App 授权。V1 不实现 OAuth App、PAT、Git URL、HTTPS 或 SSH provider；这些路径会扩大授权、配置和测试矩阵，而且无法提供 GitHub App installation 级别的权限边界。GitHub 不直接存“展开后的 skills 目录树”，而是存一个稳定的 vault 格式。
...
仓库可见性不是权限不变量，唯一 installation repository 才是 V1 的单仓库边界。后端只按 installation id、repository id、owner/repo 和 branch 校验远端身份，不读取或强制仓库可见性；`GithubRepository.private` 仅用于 Onboarding 的公开仓库风险提示，不新增 `PublicRepository` discovery 状态，也不按可见性阻止 initialize、bind、preview 或 apply。
...
- 恰好唯一 repo 时，从已校验的 `GithubRepository.private` 读取可见性：私有仓库直接继续分支与 Vault 检查；公开仓库先进入一次性确认阶段，明确说明 Skill 文件和 Git 历史对所有人可见，用户主动确认后才继续。确认不持久化，重新进入 Onboarding 时若仓库仍公开会再次提示。
- 没有仓库时提供“新建 Vault 仓库”入口，打开 GitHub 官方 `https://github.com/new`；仓库创建由 GitHub 完成，Skill Sync 不调用 `/user/repos` 等仓库创建 endpoint，也不自动把新仓库加入 installation，用户返回后重新检查。
...
- 仓库可见性不是保存 remote config 或 initialize/bind/preview/apply 的阻止条件；后端不新增 `PublicRepository` 状态，公开仓库与私有仓库走同一远端路径，公开风险提示只在前端 Onboarding 一次性确认。
