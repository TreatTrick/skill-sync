...
mod tests {
    ...
    fn apply_response_uses_status_tag() {
        ...
        let recovery = ApplySyncResponse::RecoveryRequired {
            recovery: RecoveryInfo {
                task_id: "t".into(),
                phase: RecoveryPhase::TrashMoveFailed,
                remote_commit: Some("c".into()),
                completed_action_ids: vec![],
                pending_action_ids: vec!["p".into()],
                message: "m".into(),
            },
        };
        let json = serde_json::to_value(&recovery).unwrap();
        assert_eq!(json["status"], "recovery_required");
        assert_eq!(json["recovery"]["phase"], "trash_move_failed");
        assert_eq!(json["recovery"]["pending_action_ids"][0], "p");
    }
