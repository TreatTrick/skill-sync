...
- fetch blob 后先断言 actual compressed length == manifest size 且 SHA-256 == manifest hash，再调用 Task 4 的 canonical/resource-validating `unpack_skill`。size/hash/noncanonical entry/任何 unpack limit 失败都清理 task temp，现有 target 和 sync_state bytes 不变。
