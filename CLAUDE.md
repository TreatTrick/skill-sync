...
3. Organize React code under `src/app`, `src/shared`, and `src/modules`.
4. Do not add wrappers that only forward Zustand `getState()` or setter calls.
...
8. Keep imports flowing `app -> modules -> shared`; do not create circular imports, self imports, or `export *` barrels.
