// 三方比较的 merge_plan、plan fingerprint 与 build_plan 入口：把 base/local/remote
...
use super::model::{
    BaseAdoption, BlockedSkill, CommitSummary, Conflict, ConflictReason, DeleteDirection, SyncPlan,
...
pub(crate) fn action_kind_for(status: SyncStatus) -> &'static str {
    match status {
...
pub(crate) fn validate_remote(cfg: &RemoteConfig, state: &RemoteIdentity) -> Result<()> {
